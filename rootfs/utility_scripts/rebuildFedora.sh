...
nohup \
	fedora-rebuild.sh -r org.fcrepo.server.resourceIndex.ResourceIndexRebuilder >> $CATALINA_HOME/logs/fedora-rebuild.out 2>> $CATALINA_HOME/logs/fedora-rebuild.err && \
	fedora-rebuild.sh -r org.fcrepo.server.utilities.rebuild.SQLRebuilder >> $CATALINA_HOME/logs/fedora-rebuild.out 2>> $CATALINA_HOME/logs/fedora-rebuild.err && \
	curl --user $TOMCAT_ADMIN_USER:$TOMCAT_ADMIN_PASS http://localhost:8080/manager/text/start?path=/fedora < /dev/null &
